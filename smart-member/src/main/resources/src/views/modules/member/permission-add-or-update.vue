    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.uid) {
            this.$http({
              url: this.$http.adornUrl(`/member/permission/info/${this.dataForm.uid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.permissionName = data.permission.permissionName
                this.dataForm.description = data.permission.description
                this.dataForm.createdTime = data.permission.createdTime
                this.dataForm.updatedTime = data.permission.updatedTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/member/permission/${!this.dataForm.uid ? 'save' : 'update'}`),
              method: 'post',
